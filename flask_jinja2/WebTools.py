...
def register(email_address, emailcode):
    ...
    r = s.post(url, data=formdata, headers=headers, timeout=60)
...
def login_to_user(email_address):
    ...
    resp = s.get("https://freemycloud.xyz/user", timeout=60)
    while (resp.status_code != 200):
        resp = s.get("https://freemycloud.xyz/user", timeout=60)  # 确保进入用户中心
